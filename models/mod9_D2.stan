  ...
  real mu_zt = mu_alpha * (1-exp(-focal_exponent)) / beta;
      real t1 = log( beta *  pow(eta_alpha,2));
      real t2 = log( beta *  pow(eta_alpha,2)) -2 * focal_exponent;
      ...
      real sigma_zt = sqrt((exp(t1) - exp(t2) + exp(t3) + exp(t4) - exp(t5)) / (2 * pow(beta,2)));
    ...
    for(m in 0:M)
  {
    ...
    return(log(h/3) + log_sum_exp(lp));
      ...
      vector[N] t1 = log( beta *  pow(eta_alpha,2));
      vector[N] t2 = log( beta *  pow(eta_alpha,2)) -2 * focal_exponent;
      real t3 = log(2 * pow(sigma_alpha, 2));
      vector[N] t4 = log(2 * pow(sigma_alpha, 2)) - 2* focal_exponent;
      vector[N] t5 = log(4 * pow(sigma_alpha,2)) - focal_exponent;
    
      sigma_zt = sqrt((exp(t1) - exp(t2) + exp(t3) + exp(t4) - exp(t5)) ./ (2 * pow(beta,2)));
