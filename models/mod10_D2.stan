  ...
  real f(real ntp1, real nt, real ft, real ovi_period, real mu_alpha, real beta0, real beta1, real gamma_alpha) {
  ...
  real focal_exponent = ft * ovi_period * beta;
      ...
      real sigma_zt = sqrt( pow(gamma_alpha / beta, 2) * (1 + exp(t1) - exp(t2)));
  ...
  real log_lik_Simpson_f(real ntp1, real nt, real ovi_period, real mu_alpha, real beta0, real beta1, real gamma_alpha, int M) {
    ...
    for(m in 0:M)
  {
    lp[m + 1] = get_simp_add_on(m, M) + f(ntp1, nt, li + m * h, ovi_period, mu_alpha, beta0, beta1, gamma_alpha);
  }
    ...
    return(log(h/3) + log_sum_exp(lp));
      ...
      sigma_zt = sqrt( pow(gamma_alpha ./ beta, 2) .* (1 + exp(t1) - exp(t2)));
      ...
      for(i in 1:N)
      {
        log_lik[i] = log_lik_Simpson_f(ntp1[i], nt[i], ovi_period[i], mu_alpha, beta0, beta1, gamma_alpha, M);
      }
       }
