  ...
  real f(real ntp1, real nt, real ft, real ovi_period, real mu_alpha, real beta0, real beta1, real eta_alpha, real sigma_alpha) {
  
  real beta = beta0 + nt * beta1;
  real focal_exponent = ft * ovi_period * beta;
  real mu_zt = mu_alpha * (1-exp(-focal_exponent)) / beta;
      real t1 = log(ft * beta *  pow(eta_alpha,2));
      real t2 = log(ft * beta *  pow(eta_alpha,2)) -2 * focal_exponent;
      ...
      real sigma_zt = sqrt((exp(t1) - exp(t2) + exp(t3) + exp(t4) - exp(t5)) / (2 * ft * pow(beta,2)));
  ...
  real log_lik_Simpson_f(real ntp1, real nt, real ovi_period, real mu_alpha, real beta0, real beta1, real eta_alpha, real sigma_alpha, int M) {
    ...
    for(m in 0:M)
  {
    lp[m + 1] = get_simp_add_on(m, M) + f(ntp1, nt, li + m * h, ovi_period, mu_alpha, beta0, beta1, eta_alpha, sigma_alpha);
  }
    ...
    return(log(h/3) + log_sum_exp(lp));
      ...
      mu_zt = alpha .* (1-exp(-focal_exponent)) ./ beta;
      
      vector[N] t1 = log(ft .* beta *  pow(eta_alpha,2));
      vector[N] t2 = log(ft .* beta *  pow(eta_alpha,2)) -2 * focal_exponent;
      real t3 = log(2 * pow(sigma_alpha, 2));
      vector[N] t4 = log(2 * pow(sigma_alpha, 2)) - 2* focal_exponent;
      vector[N] t5 = log(4 * pow(sigma_alpha,2)) - focal_exponent;
    
      sigma_zt = sqrt((exp(t1) - exp(t2) + exp(t3) + exp(t4) - exp(t5)) ./ (2 * ft .* pow(beta,2)));
      ...
      for(i in 1:N)
      {
        log_lik[i] = log_lik_Simpson_f(ntp1[i], nt[i], ovi_period[i], mu_alpha, beta0, beta1, eta_alpha, sigma_alpha, M); 
      }
       }
