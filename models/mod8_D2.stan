  ...
  real f(real ntp1, real nt, real ft, real ovi_period, real mu_alpha, real beta0, real beta1, real sigma_alpha) {
  ...
  real mu_zt = mu_alpha * (1-exp(-focal_exponent)) / beta;
      ...
      real sigma_zt = sqrt((-exp(t1) + exp(t2) + exp(t3)) / (ft * pow(beta,2)));
  ...
  real log_lik_Simpson_f(real ntp1, real nt, real ovi_period, real mu_alpha, real beta0, real beta1, real sigma_alpha, int M) {
    ...
    for(m in 0:M)
  {
    lp[m + 1] = get_simp_add_on(m, M) + f(ntp1, nt, li + m * h, ovi_period, mu_alpha, beta0, beta1, sigma_alpha);
  }
    ...
    return(log(h/3) + log_sum_exp(lp));
      ...
      sigma_zt = sqrt((-exp(t1) + exp(t2) + exp(t3)) ./ (ft .* pow(beta,2)));
      ...
      for(i in 1:N)
      {
        log_lik[i] = log_lik_Simpson_f(ntp1[i], nt[i], ovi_period[i], mu_alpha, beta0, beta1, sigma_alpha, M); 
      }
       }
