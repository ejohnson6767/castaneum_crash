...
tiff(here("figures/fig1.tiff"), units="in", width=15, height=5, res=300)
...
png(here("figures/fig1.png"), units="in", width=15, height=5, res=300)
ggarrange(plotlist = gl,
          nrow = 1, 
          widths = c( 3, 1,3, 1, 3),
          labels = c("(a)", NA, "(b)", NA, "(c)"),
...
fig2v1 <- annotate_figure(fig2v1, 
                left = textGrob("n(t+1)", rot = 90, hjust = 0, vjust = 0.5, gp = gpar(fontsize = axis_title_size)), bottom =textGrob("n(t)", hjust = 2, vjust = 0, gp = gpar(fontsize = axis_title_size)))
...
# save high quality tiff
tiff(here("figures/fig2.tiff"), units="in", width=15, height=5, res=300)
fig2
dev.off()

# save high quality png
png(here("figures/fig2.png"), units="in", width=15, height=5, res=300)
fig2
dev.off()
```
...
# save high quality tiff
tiff(here("figures/fig3.tiff"), units="in", width=18, height=6, res=300)
figure3
dev.off()

# save high quality png
png(here("figures/fig3.png"), units="in", width=18, height=6, res=300)
figure3
dev.off()
```
...
sim_stoch<- function(pars, input_dat)
{
  ...
  ft <- rbinom(n = N, size = nt, prob = 0.5)
  ...
  ntp1 <- round(rnormt(N, range = c(0, Inf), m = zt*mu_theta, s = sqrt(zt*mu_theta*(1-mu_theta))))
  #ntp1[which(ntp1 < 0)] <- 0
...
sfit <- readRDS(file = here("model_fits/sfit_full_D3.rds"))
...
tiff(here("figures/fig4.tiff"), units="in", width=10, height=10, res=300)
fig3_dat %>%
  ggplot(aes(x = mu_alpha, y = n)) +
  geom_point(alpha = 0.01, shape=16, size = 0.5) +
  facet_grid(treatment ~ stoch_factor, labeller = label_wrap_gen(width = 15)) +
  xlab(TeX("Oviposition rate, $\\alpha \\; \\left(eggs \\times female^{-1} \\times day^{-1}\\right)$")) +          ylab("Population abundance") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.text = element_text(size = axis_text_size),
...
png(here("figures/fig4.png"), units="in", width=10, height=10, res=300)
fig3_dat %>%
  ggplot(aes(x = mu_alpha, y = n)) +
  geom_point(alpha = 0.01, shape=16, size = 0.5) +
  facet_grid(treatment ~ stoch_factor, labeller = label_wrap_gen(width = 15)) + 
...
#   ggplot(aes(x = x)) + 
#   geom_histogram() + 
#   xlab(TeX("Oviposition rate, $\\alpha \\; \\left(eggs \\times female^{-1} \\times day^{-1}\\right)$"))
...
# save high quality tiff
tiff(here("figures/fig5.tiff"), units="in", width=15, height=5, res=300)
ggarrange(plotlist = gl,
          nrow = 1, 
          labels = c("(a)", "(b)", "(c)"),
          hjust = -1,
...
# save high quality png
png(here("figures/fig5.png"), units="in", width=15, height=5, res=300)
ggarrange(plotlist = gl,
          nrow = 1, 
          labels = c("(a)", "(b)", "(c)"),
          hjust = -1,
...
# save high quality tiff
tiff(here("figures/figS1.tiff"), units="in", width=10, height=7, res=300)
S1
dev.off()

# save high quality png
png(here("figures/figS1.png"), units="in", width=10, height=7, res=300)
S1
dev.off()
...
# save high quality tiff
tiff(here("figures/figS2.tiff"), units="in", width=10, height=10, res=300)
figS2
dev.off()

# save high quality png
png(here("figures/figS2.png"), units="in", width=10, height=10, res=300)
figS2
dev.off()
```
...
# make tiffs
tiff(here("figures/figS3.tiff"), units="in", width=10, height=7, res=300)
S3
dev.off()

tiff(here("figures/figS4.tiff"), units="in", width=15, height=7.5, res=300)
S4
dev.off()

tiff(here("figures/figS5.tiff"), units="in", width=15, height=7.5, res=300)
S5
dev.off()

# make pngs
png(here("figures/figS3.png"), units="in", width=10, height=7, res=300)
S3
dev.off()

png(here("figures/figS4.png"), units="in", width=15, height=7.5, res=300)
S4
dev.off()

png(here("figures/figS5.png"), units="in", width=15, height=7.5, res=300)
S5
dev.off()
