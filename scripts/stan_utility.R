# 7/4/2021 modified from Betancourt github so that check_rhat and check_neff take a pars argument - a vector of parameter names 
...
check_n_eff <- function(fit, pars) {
  fit_summary <- summary(fit, probs = c(0.5), pars = pars)$summary
  N <- dim(fit_summary)[[1]]
