...
# select dataset 2
Also, subset so that oviposition days ~ 7.
```{r}
# subset 
dat <- all_dat %>%
...
# the oviposition period was slightly longer than 7 days for the "castaneum_chaos" experiment, hence the >= 7. Aggregating data with two different oviposition periods is justifiable because the oviposition periods are very similar (7.083 vs 7.0).
...
# tune initial guesses of parameter values: multi-modal HDPrI
```{r}
...
sfit <- readRDS(file = here("model_fits/sfit1_D2.rds"))
...
outer_perc <- 0.99 # the percentile for the outer predictive interval
...
write('
      ...
      real <lower = 0> beta1; // effective cannibalism rate slope
      ...
      real <lower = 0> gamma1; // the slope of the die-off threshold
      ...
      vector<lower = 0>[N] beta; // the effective cannibalism rate
      vector[N] mu_zStar; // the average die-off threshold
      ...
      beta = beta0 + nt * beta1; // effective cannibalism rate
      mu_zStar =  gamma0 - gamma1*nt; // the average die-off threshold
      zStar = mu_zStar + sigma_zStar * zStar_raw;

      // adult-to-egg model
      focal_exponent = ft .* ovi_period .* beta; // useful intermediate quantity
      mu_zt = mu_alpha * (1-exp(-focal_exponent)) ./ beta; // average number of eggs
      sigma_zt = eta_alpha * sqrt( (1- exp(-2 * focal_exponent)) ./ (2 * beta)); // the standard deviation in the number of eggs
      ...
      ',
        ...
        here("mod1_full_D2.stan"))

# compile stan model
mod1_full_D2 <- cmdstan_model(here("mod1_full_D2.stan"))
...
crash_inds <- which(dat$nt > 25 & dat$ntp1 < 150) # indexes of replicates that experienced population crashes
...
# fit model
fit <- mod1_full_D2$sample(data = stan_data, 
                   iter_warmup = warmup, 
                   iter_sampling = iter,
                   chains = chains,
...
saveRDS(sfit, file = here("model_fits/sfit_full_D2.stan"))
...
par_names <- c("sigma_zStar", "thresh_length", "gamma0", "gamma1", "epsilon_H")
par_names <- c("m", "mu_alpha", "beta0", "beta1", "eta_alpha")
x <- as.array(sfit, pars = par_names)
mcmc_pairs(x, condition = pairs_condition(chains = list(3, NULL)))

# select the best chain(s)
best_chain <- c(1,2,3,5,6)
...
par_names <-   c("m", "beta0", "beta1", "eta_alpha", "thresh_length", "gamma0", "gamma1", "sigma_zStar", "epsilon_H")
...
par_names <- c("mu_alpha", "beta0", "beta1", "eta_alpha", "thresh_length", "gamma0", "gamma1", "sigma_zStar", "epsilon_H") # parameter names
...
saveRDS(panel, file = here("figures/fig2_panelB.rds"))
```
